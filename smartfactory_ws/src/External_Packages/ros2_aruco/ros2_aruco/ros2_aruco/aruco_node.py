...
class ArucoNode(rclpy.node.Node):
    def __init__(self):
        ...
        # Declarar e ler parâmetros
        self.declare_parameter(
            name="marker_size",
            value=0.035, #0.0625, tamanho atual dos marcadores 
        ...
        # Certifique-se de que temos um ID de dicionário válido:
        try:
        ...
        self.aruco_parameters.maxMarkerPerimeterRate = 0.2
        ...
        # Configuração dos publicadores
        self.poses_pub = self.create_publisher(PoseArray, "aruco_poses", 10)
        self.markers_pub = self.create_publisher(ArucoMarkers, "aruco_markers", 10)
        self.image_pub = self.create_publisher(Image, "aruco_image", 10)
        self.vis_pub = self.create_publisher(Marker, "aruco_marker_vis", 10)

        # Configuração dos campos para parâmetros da câmera
        self.info_msg = None
    ...
    def image_callback(self, img_msg):
        ...
        if marker_ids is not None:
            ...
            for i, marker_id in enumerate(marker_ids):
                ...
                quat = tf_transformations.quaternions.mat2quat(rot_matrix[:3, :3])
                ...
                # Anotar a imagem com o ID do marcador e o limite
                cv_image = cv2.aruco.drawDetectedMarkers(cv_image, corners)
                cv2.putText(cv_image, f"ID: {marker_id[0]}", (int(corners[i][0][0][0]), int(corners[i][0][0][1]) - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv2.LINE_AA)
                
                self.get_logger().info(f"Detected marker ID: {marker_id[0]}, Pose: {pose}")

                # Publicar um marcador visual para o RViz
                marker = Marker()
                marker.header.frame_id = markers.header.frame_id
                marker.header.stamp = markers.header.stamp
                ...
                marker.scale.z = 0.1  # Tamanho do texto
                marker.color.a = 1.0  # Transparência
                marker.color.r = 1.0  # Cor (vermelho)
                marker.color.g = 0.0
                marker.color.b = 0.0
                marker.text = f"ID: {marker_id[0]}"
                self.vis_pub.publish(marker)

            self.poses_pub.publish(pose_array)
            self.markers_pub.publish(markers)
            
        # Publicar a imagem anotada
