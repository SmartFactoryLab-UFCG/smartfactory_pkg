...
class ArucoPoseWorld(Node):
    ...
    def __init__(self):
        super().__init__('basler_aruco_pose_world')
        ...
        self.pose_subscriber = self.create_subscription(
            PoseArray, 'basler/aruco/poses', self.pose_callback, 10)

        # Publicador da pose do Aruco em relação ao mundo
        self.pose_world_publisher = self.create_publisher(PoseArray, 'basler/aruco/poses_world', 10)
        ...
        for i, pose in enumerate(msg.poses):
            try:
                ...
                transform_camera_world = self.tf_buffer.lookup_transform(
                    'world', 'camera_rgb_frame_basler', rclpy.time.Time())
                ...
                t_world.child_frame_id = f"basler_aruco_world_{i}"
                ...
                pose_world.position.z = t_aruco_w_translation[2] + 0.065
